...
const Android = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        const [ov, ap, inv, qqnt] = await Promise.allSettled([
        ...
        if (![ov, ap, inv, qqnt].some((result) => result.status === 'fulfilled' && result.value)) {
    ...
    return (
      <div className="space-y-6">
        <div>
          ...
          <p className="mt-2 text-slate-600">解析小米 MIUI 离线备份（descript.xml + .bak），还原设备信息、应用与 QQNT 证据</p>
        </div>
        <Card title="选择任务">
