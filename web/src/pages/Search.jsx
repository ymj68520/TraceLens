...
const Search = () => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <motion.h1 initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="text-3xl font-bold text-slate-900 dark:text-white">Full-Text Search</motion.h1>
        <p className="mt-2 text-slate-600 dark:text-slate-300">Search indexed file contents</p>
        {currentTask && (
          <div className="mt-2">
        ...
        <nav className="-mb-px flex space-x-8" aria-label="Tabs">
          <button
            ...
            className={`${activeTab === 'search'
              ? 'border-blue-500 text-primary-600 dark:text-primary-400 dark:border-blue-400'
          ...
          <form onSubmit={handleSearch} className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-500 dark:bg-slate-700 dark:text-white dark:placeholder-gray-400"
          ...
          <form onSubmit={handleCreateIndex} className="space-y-4">
            ...
            <div className="flex items-center">
              <input
                ...
                checked={true}
                disabled
                className="h-4 w-4 text-primary-600 focus:ring-primary-500 border-slate-300 rounded dark:bg-slate-700 dark:border-slate-600"
              />
              <label htmlFor="recursive" className="ml-2 block text-sm text-slate-900 dark:text-white">
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              {results.results.map((result, idx) => (
                <div key={idx} className="p-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <p className="font-mono text-sm text-slate-900 dark:text-white break-all">{result.path || result.file_path}</p>
                      ...
                      {result.snippet && (
                        <div className="mt-2 p-3 bg-yellow-50 dark:bg-yellow-900/10 border-l-4 border-yellow-400 dark:border-yellow-600">
                          <p className="text-sm text-slate-700 dark:text-slate-300 font-mono">{result.snippet}</p>
                        </div>
                      )}
                    </div>
                  </div>
