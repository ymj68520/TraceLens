...
export const ARTIFACT_COLUMNS = {
  ...
  apps: [
    ...
    { key: 'data_size', label: '数据大小', format: fileSize },
    { key: 'sd_size', label: 'SD卡大小', format: fileSize },
    { key: 'bak_type', label: '备份类型' },
    { key: 'manifest_summary', label: '清单摘要' },
  ],
  // MIUI / Android-backup source inventories. These are what a backup carries
  // in place of live app data, so they are the evidence for those images.
  app_db_inventory: [
    { key: 'package_name', label: '包名' },
    { key: 'db_path', label: '数据库路径' },
  ...
  qqnt_artifacts: [
    ...
    { key: 'bak_file', label: '备份文件' },
    { key: 'artifact_category', label: '类别' },
    { key: 'format', label: '格式' },
    { key: 'size', label: '大小', format: fileSize },
    { key: 'modified_time', label: '修改时间', format: fmtTime },
  ...
  qqnt_kv: [
    ...
    { key: 'value_text', label: '值' },
    { key: 'is_sensitive', label: '敏感', format: (v) => Number(v) === 1 ? '是' : '否' },
    { key: 'parse_status', label: '解析状态' },
    { key: 'source_path', label: '源路径' },
  ],
  wechat_artifacts: [
