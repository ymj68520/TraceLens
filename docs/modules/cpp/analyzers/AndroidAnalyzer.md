...
入口流程（`AndroidAnalyzerCore.cpp:61-148, 150-258`）：`initialize()` 选后端并建库 → `analyzeAndroidData()` 按固定清单依次解析（见第 3 节）→ 最后 `analyzeWithLLM()` 做工件级 AI 分析。MIUI 模式还有一个贴心的自动晋升：用户把 MIUI 备份目录当成普通"目录"源提交时，`initialize()` 检测到 `descript.xml` 根标签是 `<MIUI-backup>` 且有 `.bak` 文件就自动切到 miui-backup 模式（`AndroidAnalyzerCore.cpp:61-73`）。
...
**链路一：短信从镜像到 sms_messages 表。** `analyzeAndroidData()` 调 `extractAndParseDB("data/data/com.android.providers.telephony/databases/mmssms.db", "parseSMS")`（`AndroidAnalyzerCore.cpp:170`）。`extractAndParseDB`（`AndroidDataParsers.cpp:42-79`）先 `makeAnalysisTempPath` 生成临时路径，`stageSqliteBundle` 通过当前后端把库（和边车）复制出来，然后按字符串分派到 `parseSMS`。`parseSMS`（第 81-109 行）直接 `sqlite3_open` 打开临时副本，`SELECT address, body, date, type FROM sms` 逐行读出，经 `insertSMS` 参数化插入。解析完立刻删临时文件——原始证据从头到尾只读不写。

**链路二：微信 EnMicroMsg.db 的解密。** 非 MIUI 路径固定取 `data/data/com.tencent.mm/MicroMsg/testuser/EnMicroMsg.db`（`AndroidAnalyzerCore.cpp:224`），暂存后交给 `parseWeChatEnhanced(tempPath, wechatPassword_)`。密码来源有三层：`--wechat-password` 参数显式给；不给时 `WeChatDecryptor::derivePassword` 从镜像里找 UIN——读 `com.tencent.mm/shared_prefs/` 下两个 XML 里的 `_auth_uin`/`default_uin` 属性，IMEI 无从获取时用占位值，拼起来做 `MD5(IMEI+UIN)` 取前 7 位（`WeChatDecryptor.cpp:62-111`，中间量用 `OPENSSL_cleanse` 清零）。拿到密码后 `openDatabase`（第 28-60 行）按"SQLCipher v4 默认 → v2 传统参数（4000 轮 KDF + SHA1 HMAC）→ 全版本矩阵"的顺序尝试打开。打开成功后走 `parseWeChatContacts/parseWeChatChatrooms/identifyWeChatOwner` 加增强消息解析（声明在 `AndroidAnalyzerDeclarations.h:166-169`），分别落 wechat_contacts、wechat_chatrooms、wechat_owner_info、wechat_messages 表。MIUI 模式则改成枚举备份里所有微信库成员逐个处理（`AndroidAnalyzerCore.cpp:185-222`）。

**链路三：MIUI 备份的清单化与库盘点。** `initialize()` 识别 miui-backup 模式后立刻 `persistMiuiBackupAnalysis`（`AndroidAnalyzerCore.cpp:137-144`）：`writeMiuiManifest` 把 descript.xml 解析成设备/MIUI 版本/包列表，写 miui_backup_manifest 和每个包一行 installed_apps；`writeAppDbInventory` 遍历 `apps/<包名>/db/` 下的每个 SQLite，能读的逐表记行数与列名，读不了的（加密/损坏）也写一行失败记录而不是静默跳过（`MiuiArtifactParsers.h:16-22` 注释），整个过程在一个事务里，失败不会留下"看似完整的半成品"（同文件第 25-26 行）。随后 QQNT/微信的专属解析器（`persistQqntBackupAnalysis` 等）在此之上补充结构化记录。这个盘点有大量防失控限额（单库 512MB、清单 1 万行、候选 10 万个，见 `MiuiArtifactParsers.cpp:32-50`），防止一个畸形备份拖死分析。
