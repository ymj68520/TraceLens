...
**（a）两套存储，一个根目录。** 装配公式在 service_manager.py:392-415：`root = FORENSIC_REPORT_DIR`（默认 `build/data/reports`，config.py:214-216），`ReportRepository(root / "reports.db")` + `SnapshotWriter(root / "snapshots", FORENSIC_REPORT_GENERATOR_VERSION)` + `build_default_adapters()`。`report_versions` 表（repository.py:35-53）以 `UNIQUE(scope_type, scope_id, version)` 记录 queued→generating→ready/failed；`report_generation_inputs` 表（:68-89）是 R2b 冻结准入行，靠四个触发器硬约束：

- `trg_report_generation_input_frozen`（:92-107）：generation_id/task/scope/requester/envelope/hash/created_at 一旦写入禁止 UPDATE；
- `trg_report_generation_input_no_delete`（:108-112）：禁止删除；
- `trg_report_generation_status_transition`（:122-131）：只许 `admitted→running|failed`、`running→completed|failed`；
- `trg_report_generation_terminal_immutable`（:153-169）：终态行不可再改。

`report_kind` 列（:198-217）区分确定性快照（NULL）与 LLM 叙事（`llm_generation`，一次性回填 `manifest_path LIKE 'snapshots/%'` 的行）。版本分配用 `BEGIN IMMEDIATE + MAX(version)+1 + INSERT` 单事务（:234-258、:511-535），`_assert_mutable`（:264-271）保证 ready/failed 版本不可变。
...
- **final_report_* 链当前无路由入口**：装配/发布/展示的编排方法全部挂在 legacy `InvestigationService` 类上（investigation_service.py:1378-1600），该类没有任何路由实例化；Workbench 的 `/final-reports` 端点实际读的是 R2 narrative manifest（investigation_workbench.py:160-212）。相关机制仍由单测直接覆盖（见第 7 节）。
