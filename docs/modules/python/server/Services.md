...
- **agent↔server 端点↔PG 表对应**：poller → `GET /api/commands/poll` → clients（last_poll/last_seen/status）+ command_queue（领取）；command_executor → 状态端点 `POST /api/commands/{id}/status` → command_queue + analysis_tasks + task_history；result_uploader/index_uploader → `POST /api/tasks/{id}/results` → analysis_results；LLM 回收 → llm_analysis。

## 3. 核心接口清单

| 方法（真实签名） | 语义 | 调用方 | 失败行为 |
|---|---|---|---|
| `create_user_token(user_id, org_id, role, permissions) -> str` | 签 1h user JWT | auth 路由（登录/注册） | 不失败 |
| `create_client_token(client_id, org_id, capabilities) -> str` | 签 30d client JWT | 客户端注册路由 | 不失败 |
| `verify_token(token) -> Optional[dict]` | 验签解码 | 中间件 | 过期/无效一律 None |
| `get_user_from_token(token, db)` / `get_client_from_token(token, db)` | 验签 + 按 id 回查 ORM | 中间件 | 类型不符/主体已删→None |
| `CommandQueueService.create_command(command_data, user_id, db) -> CommandQueue` | 入队（critical 强制 1h TTL） | commands/tasks 路由 | 客户端不存在 ValueError |
| `CommandQueueService.get_pending_commands(client_id, db) -> List[CommandQueue]` | 优先级领取并转 assigned | poll 组装 | 不抛 |
| `CommandQueueService.update_command_status(command_id, status, result_message=None, db=None)` | 状态上报语义（盖时间戳/retry_count） | 状态路由 | 命令不存在 ValueError |
| `CommandQueueService.expire_commands(db=None) -> int` | TTL 清扫 pending/assigned→expired | expire 端点 | 不抛 |
| `CommandQueueService.get_commands_for_client(client_id, db) -> CommandPollResponse` | 刷新在场 + 领取（一次 poll） | poll 路由 | 不抛 |
| `CommandQueueService.propagate_command_status(db, command, status, progress=None, message=None)` | 命令→任务桥（best-effort） | 状态路由 | ValueError 只记 warning |
| `TaskOrchestrator.create_analysis_task(task_data, user_id, db) -> AnalysisTask` | 一事务建任务+命令+history | tasks 路由 | 校验失败 ValueError |
| `TaskOrchestrator.update_task_progress(...)` / `complete_task(...)` | 推进/终态（终态幂等护栏） | propagate | 越界 ValueError |
| `TaskOrchestrator.cancel_task(task_id, user_id, db)` | 取消并失败未开跑命令 | 取消路由 | 同上 |
| `ResultAggregator.store_result(...)` / `store_results(task_id, client_id, results, db=None)` | 单条/批量按引用登记工件 | results 路由 | 坏 result_type/非本任务 ValueError |
| `ResultAggregator.store_llm_analysis(task_id, analysis_result, ...)` | 存 LLM 回收记录 | LLM 回收路由 | 任务不存在 ValueError |
...
```python
# server/services/auth_service.py:73-83
payload = {
...
client token 则是 `client_id`/`capabilities`、30 天时效、`type: "client"`（auth_service.py:86-116）。UUID 串化（`str(user_id)`）是因为 JWT payload 只收 JSON 标量。`type` 字段是中间件互斥校验的依据（client token 打 user 端点 → 401）。`verify_token` 对过期/无效统一返回 None（auth_service.py:129-135，不区分错误类别以免泄露探测面）；`get_user_from_token` / `get_client_from_token` 验签后再按 id 回查 ORM——token 有效但主体已删的边界被兜住。口令是 passlib bcrypt（auth_service.py:38-48）。
...
```python
# server/services/command_queue.py:109-140（节选）
now = _now()
...
`case()` 把字符串优先级映射成整数序（未知值 else_=5 排最后），同优先级按 created_at 老优先；TTL 过滤在领取时兜底（过期命令即使没被清扫也不会被领走）。`create_command`（:41-91）对 `critical` 命令**无视调用方给的 ttl_hours**，强制 1 小时短窗（`CRITICAL_COMMAND_TTL_HOURS`，其余默认 24h）；显式置 `status="pending"` 而不依赖列默认，保证返回对象在 flush 前就完整。`expire_commands`（:206-246）是 TTL 清扫器：pending/assigned 且 TTL 已过的批量转 `expired`：

```python
# server/services/command_queue.py:227-241（节选）
expired_commands = (
    db.query(CommandQueue)
    .filter(
        and_(
...
`update_command_status`（:144-204）的语义是：`completed`/`failed` 盖 `completed_at`，`failed` 额外 `retry_count += 1`；`in_progress` 只翻状态；`result_message` 仅在新值提供时覆写，进度上报不会冲掉既有错误信息。`get_commands_for_client`（:248-288）组装一次 poll：先刷新在场状态（`last_seen=now`；`last_poll` 在 60s 内则 online，否则 offline——`last_poll` 由 poll 路由在调它之前盖章，api/commands.py:100-113），再领取命令。

**（c）propagate_command_status：命令→任务的桥。** 命令状态上报后，此方法（command_queue.py:290-385）把变化传导给发起任务。task 解析与防御：

```python
# server/services/command_queue.py:336-355（节选）
...
**（e）result_aggregator：按引用回收，不传字节。** 工件契约（result_aggregator.py:65-74、136-144）明确：`file_path`/`storage_location` 是客户端给的**不透明句柄**，服务端不假设本地模式的固定文件名（raw.db/events.db/files.db 是 :8666 C++ 进程内约定，不适用分布式路径）；原始镜像字节永不过线，只登记引用与元数据，`result_metadata` 原样落库（其中的 `base_name` 是客户端命名的工件基名，形如 `<baseName>_<kind>.db`）。`store_results`（:126-197）批量版**先全量校验后写**：

```python
# server/services/result_aggregator.py:165-191（节选）
...
先验后写保证坏批次不落半个；`client_id` 统一取 `task.client_id`（而不是上传者自报），所有权经 `_get_task_or_raise`（:312-330）双重检查：client 只能往**自己任务**里塞结果，兄弟客户端注入被挡。`store_llm_analysis`（:203-262）存 agent 本地 LLM 分析的回收记录（`input_text_hash` 是去重键，`cost` 是 Numeric(10,4)）。两个模型的时间戳都交给 `server_default=func.now()`，服务层不设时间（result_aggregator.py:26-30）。

## 5. 工作流程走读：一个任务的完整生命周期
...
- 结果回收的并发语义：同任务并发上传两个批次各自整体提交，行级无冲突（独立 UUID）；但"重复上传同一工件"会产生重复行——幂等性由客户端保证（input_text_hash 仅是 llm_analysis 的去重键而非唯一约束）。
