...
- [ ] 练习 1：同一关键词分别用布尔/短语/通配三种写法，对比命中数差异并解释。
- [ ] 练习 2：用 path:/ext: 前缀各完成一次定向检索。
- [ ] 练习 3：对同目录二次索引，验证幂等（命中数不变）。
- [ ] 扩展实验 A：filter-then-index 策略实测——全量 vs 过滤后索引的耗时对比。
- [ ] 扩展实验 B：搜索定位 3 个文件 → 逐个重分析 → 用 SQL 食谱第 3 条核对落库。
- [ ] 思考题：FTS_ALLOWED_ROOT 之外发起 index 会怎样？为什么这样设计？
