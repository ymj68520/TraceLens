...
| 维度 | A 单机 all-in-one | B 单机分进程（默认 run.sh） | C 分布式 C/S |
|------|------------------|---------------------------|--------------|
| 适用 | 个人分析工作站 | 小团队内网共享一台工作站 | 多取证机集中管理 |
| 认证 | 无（靠隔离） | 无（靠隔离） | JWT+组织 |
| 数据 | 本机 data/ | 本机 data/ | PG+agent 本地产物上传登记 |
| 前置 | setup.sh 全套 | +Node/Neo4j/Redis | +PG+每机编译 agent |
| 运维成本 | 低 | 中（Monitoring 三通道） | 高（升级/迁移手册） |

选择树：只有一个人用→A；多人同时用/要图谱报告→B；取证机分散多地→C（可 B+C 共存，端口天然分离）。
