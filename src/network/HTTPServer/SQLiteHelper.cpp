...
json SQLiteHelper::get_android_communication_summary(const std::string& android_db) {
    ...
    // SMS summary
    json sms_summary = execute_query(db, "SELECT COUNT(*) as total_sms FROM sms_messages");
    json sms_by_type = execute_query(db, "SELECT type, COUNT(*) as count FROM sms_messages GROUP BY type");

    // WhatsApp summary
    json whatsapp_summary = execute_query(db, "SELECT COUNT(*) as total_messages FROM whatsapp_messages");
    json whatsapp_by_type = execute_query(db, R"(
        SELECT CASE
    ...
    // Contacts
    json contacts_summary = execute_query(db, "SELECT COUNT(*) as total_contacts FROM contacts");

    // Call logs
    json call_summary = execute_query(db, "SELECT COUNT(*) as total_calls FROM call_logs");
    json call_by_type = execute_query(db, "SELECT type, COUNT(*) as count FROM call_logs GROUP BY type");

    result["sms_summary"] = sms_summary;
    result["sms_by_type"] = sms_by_type;
    result["whatsapp_summary"] = whatsapp_summary;
    result["whatsapp_by_type"] = whatsapp_by_type;
    result["contacts_summary"] = contacts_summary;
    result["call_summary"] = call_summary;
    result["call_by_type"] = call_by_type;
...
json SQLiteHelper::get_android_app_usage(const std::string& android_db) {
    ...
    // Usage stats
    json usage_stats = execute_query(db, R"(
...
json SQLiteHelper::get_android_device_info(const std::string& android_db) {
    ...
    // Build properties
    json build_props = execute_query(db, "SELECT property_name, property_value FROM system_build_properties");

    // WiFi networks
    json wifi_networks = execute_query(db, R"(
    ...
    // Chrome history
    json chrome_history = execute_query(db, R"(
    ...
    result["build_properties"] = build_props;
    result["wifi_networks"] = wifi_networks;
